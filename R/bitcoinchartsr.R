...
# =======================================================================================================
download_daily_dump <- function(symbol, data.directory, overwrite=FALSE) {
  full.path <- paste(data.directory, '/', symbol, '-dump.csv', sep='')
  if(file.exists(full.path)) {
    if(!overwrite) {
      print(paste(full.path, 'already exists, skipping download...'))
      return(full.path)
    } else {
      print(paste('Deleting old dump file', full.path))
      unlink(full.path)
    }
  }
...
# =======================================================================================================
download_all_daily_dumps <- function(base.dir='data', overwrite=FALSE) {
  tryCatch(expr={
    ...
    sapply(all.symbols, function(x) {
      ...
      if(!file.exists(dir.name)) {
        print(paste('Creating data dir', dir.name))
        dir.create(dir.name, showWarnings=TRUE)
      }
      tryCatch(expr={
        file.name <- download_daily_dump(symbol=x, data.directory=dir.name, overwrite=overwrite)
        print(paste('Downloaded', file.name, '...'))
      }, error=function(e) {
        print(paste('Failed to download data for symbol:', x))
        print(paste('Error:', e))
      })
    })
  }, error=function(e) {
    print(paste('ABORTING! Failed to get symbol listing because of error:', e))
  })
}
...
# =======================================================================================================
get_symbol_listing <- function(markets.url='http://bitcoincharts.com/markets/', local.data.file='data/bitcoincharts-markets.csv') {
  tryCatch(expr={
    print('Getting updated symbol list from bitcoincharts.com...')
    ...
    print(paste('Failed to consult online listing, error:', e))
...
# =======================================================================================================
get_most_recent_trade <- function(symbol, data.directory) {
...
# =======================================================================================================
prepare_historical_data <- function(symbol, data.directory, download.daily.dump=FALSE) {
  # delete the lock file
  unlink(paste(data.directory, 'done.lck', sep='/'), force=TRUE)
  # first delete all existing data in the directory
  print(paste("Removing old processed data in directory (leaving dump file intact)", data.directory))
  ...
  sapply(f, FUN=function(x) { 
    last.date <- unlist(strsplit(system(command=paste('tail -n 1 ', x), intern=TRUE),split=','))[1]
    first.date <- unlist(strsplit(system(command=paste('head -n 1 ', x), intern=TRUE),split=','))[1]
  ...
  results<- sapply(f, FUN=function(x) { 
    ...
    try(silent=TRUE, expr={
      ...
      if(length(grep(last.date, x=f)) > 1) {
        ...
        print('=========================================================\n')
        print(paste('Redistributing ticks in:', files))
        ...
        x.new <- paste(x.new, '.csv', sep='')
        print(paste('Creating new file', paste(data.directory, x.new, sep='/')))
...
# =======================================================================================================
get_ticker <- function(symbol, end = '', data.directory) {
  ...
  last.line <- system(command=paste('tail -n 1 ', full.path), intern=TRUE)
  first.line <- system(command=paste('head -n 1 ', full.path), intern=TRUE)
...
# time period
# =======================================================================================================
get_trade_data <- function(symbol, start.date, end.date='', data.directory) {
  ...
  print("Will gather tick data from the following files:")
  ...
  for(fl in f[ start.idx:end.idx  ]) {
    print(paste('Getting data from',fl))
    new.rows <- ''
    tryCatch( expr= { 
...
# ==============================================================================================
get_bitcoincharts_data <- function(symbol, data.directory, start.date=as.character((Sys.Date() - months(1))), end.date=as.character(Sys.Date() + days(1)), ohlc.frequency = 'hours', download.data = TRUE, auto.assign=FALSE, environment=.GlobalEnv) {
  call <- match.call()
  ...
  print(paste('Preparing historical data for: ', symbol, sep=''))
  #   browser()
  ...
  prepare_historical_data(symbol=symbol, data.directory=data.directory, download.daily.dump=download.data)       
  #   }
  print(paste('Getting tick data for: ', symbol, sep=''))
...
# ==============================================================================================
load_all_data <- function(data.base.dir='data', start.date=as.character(Sys.Date() - months(1)), end.date=as.character(Sys.Date() + days(1)), ohlc.frequency='daily', download.data=FALSE) {
  all.symbols <- get_symbol_listing()
  dev.null <- sapply(all.symbols, function(x) {
    ...
    tryCatch(expr={
      ...
      print(paste('Failed to load data for', x, 'error:', e))
    })
  })
}